...
{
    //Sorted on basis on new calculated fValue
	return a.first.first>b.first.first;
}
};
// Djisktra to find the most optimal path that a Car should follow for least traffic.
int Two_Cost_Djisktra(int grid[][COL], Pair src, Pair dest,Car&a,priority_queue<ppPair,vector<ppPair>,TrafficCompare>openList)
{
...
//IF THE CAR MOVES EAST-
		...
		if (isValid(i, j + 1) == true) {
			...
			// If the successor is already on the closed list or if it is blocked, then ignore it.
			// Else do the following
			else if (closedList[i][j + 1] == false && isUnBlocked(grid,i-1,j)) {
...
//IF THE CAR MOVES NORTH-EAST -
		...
		if (isValid(i - 1, j + 1) == true) {
			...
			// If the successor is already on the closed list or if it is blocked, then ignore it.
			// Else do the following
			else if (closedList[i - 1][j + 1] == false && isUnBlocked(grid,i-1,j)) {
				gNew = cellDetails[i][j].g +grid[i-1][j+1];
...
void printPath(Car a,unordered_map<pair<int, int>, string, hash_pair>m)
{
	...
	cout<<"("<<a.cur_pos.first<<","<<a.cur_pos.second<<")"<<"("<<m[a.cur_pos]<<")";
	while(!a.path.empty())
	{
		Pair b=a.path.top();
		a.path.pop();
		if(m.find(b)!=m.end())
			cout<<"->"<<"("<<b.first<<","<<b.second<<")"<<"("<<m[b]<<")";
		else
...
int main()
{
	...
	m[make_pair(2,3)]="Kendriya Vidyalaya-4";
	...
	while(x!=-1)
	{
		cout<<"1.If u wish to see the current traffic press 1"<<endl;
		cout<<"2.If you want to add another car press 2"<<endl;
		cout<<"3.Else if you want to quit press 3"<<endl;
		cin>>x;
		if(x==3)
			break;
		else if(x==1)
		...
		{
			cout<<"Choose the source and destination of car from the given list of locations in Jaipur"<<endl;
			...
			a.cur_pos=src1;
			a.dest=dest1;
			ans=Two_Cost_Djisktra(grid, a.cur_pos,a.dest,a,openList);
			if(ans==1)
			{
				cout<<"Car has reached Destination"<<endl;
				...
				cout<<"Car cannot reach Destination as road is blocked"<<endl;
		}
		else 
		{
